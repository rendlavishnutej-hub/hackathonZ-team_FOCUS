...
import { genAI, getModel } from '@/lib/gemini';
...
export async function POST(request: Request) {
  ...
  const { data: file } = await supabase.from('quiz_files').select('*').eq('id', fileId).single();
  ...
  if (!forceRegenerate) {
    const { data: existing } = await supabase.from('quiz_questions').select('*').eq('fileId', fileId).eq('difficulty', difficulty);
    if (existing && existing.length >= questionCount) {
      const shuffled = [...existing].sort(() => Math.random() - 0.5).slice(0, questionCount);
      return NextResponse.json({ questions: shuffled, cached: true });
    }
  }
...
Document content:
${(file.content || '').substring(0, 14000)}`;

    try {
      let result;
      if (file.rawContent && file.type === 'application/pdf') {
        result = await model.generateContent([prompt, { inlineData: { data: file.rawContent, mimeType: 'application/pdf' } }]);
      } else {
